...
- User calls openDeposit(planId, amount).
- Contract checks: plan is enabled, amount is within min/max limits.
- Contract transfers tokens from user to itself (holds the principal).
- Contract mints an NFT to the user with a unique depositId.
- Deposit status is set to Active. maturityAt = block.timestamp + tenorDays \* 86400.

The APR and penalty are snapshotted at this moment.

Future plan updates do not affect this deposit.
...
- User calls renewDeposit(depositId, newPlanId) on or after maturityAt.
- Contract calculates the interest earned on the old deposit.
- New principal = old principal + interest (interest is compounded into the principal).
- A new deposit NFT is minted using the new plan's rate.
- Old deposit status is set to ManualRenewed.
...
Grace period example: maturityAt = Day 90. Grace period ends Day 93.

If user does not act by Day 93, the bot triggers auto-renew.

The user is protected - they keep their original APR for the renewed term.
...
- APR and penalty are snapshotted at deposit open. Admin changes to a plan never affect existing deposits.
- Interest uses simple interest only - no compounding within a single deposit term.
- Early withdrawal gives zero interest. Penalty goes to feeReceiver.
- Auto-renew preserves the original APR, protecting the user from rate decreases.
- Interest is always paid from the vault. Base rule: if the vault has insufficient funds, the withdrawal must revert. (A better design is possible - see Creative Challenge C1 in Section 8.3.)
- When paused, no withdrawals or renewals are allowed (emergency protection).
- Admin cannot alter a deposit that is already open.
...
- Transferable certificate: The deposit NFT can be transferred. If Alice sells her NFT to Bob before maturity, who can withdraw - Alice or Bob? Is this behavior good or dangerous? Show the exact line in your code that decides this.
- Empty vault: A user reaches maturity but the vault does not have enough money for the interest. The spec says "revert". What problem does this create for the user, and what alternative design could you offer (for example: pay principal only, or wait in a queue)? Which one did you choose to follow, and why?
- Dead bot: The auto-renew bot goes offline for one month. What happens to deposits that passed the grace period? Does the user lose anything? Propose one change that protects the user in this case.
- Rounding dust: The interest formula uses integer division, so some tiny amount is always lost to rounding. In your design, who keeps this dust - the user or the vault? Can the rounding ever cause a revert or a wrong balance? Prove your answer with one of your test cases.
- Boundary times: At the exact second of maturityAt, is a withdrawal "early" or "at maturity"? At the exact end of the grace period, can the user still manually renew? Show the comparison operators (>= or >) you used, and explain each choice.
- Disabled plan with active deposits: The admin disables a plan while many deposits from that plan are still active. What can those users still do? Can they still manually renew INTO the disabled plan? Justify your rule.
- Attack thinking: Describe one realistic attack on your system (for example: reentrancy on withdraw, double withdraw, or a fake token) and show the exact mechanism in your code that stops it.
...
Precision tip: always compute (a \* b \* c) / d not (a / d) \* b \* c

The first keeps precision; the second can truncate to zero for small values.
